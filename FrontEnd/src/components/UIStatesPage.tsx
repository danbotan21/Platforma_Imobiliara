...
const trustLevels: Array<[string,string]> = [
  ['verified','Verificat prin document'],['owner','Confirmat de proprietar'],['tenant','Confirmat de chiriaș'],['community','Raportat de comunitate'],['estimate','Estimare'],['unverified','Neverificat'],['disputed','Contestat'],
]
...
export function UIStatesPage({ navigate, notify }: SharedPageProps) {
  ...
  return <div className='ui-states-page'>
    ...
    <section className='section states-showcase'><div className='container'>
      <StateGroup id='state-0' label='01 · Încărcare' title='Conținutul își păstrează structura în timp ce se pregătește'><div className='state-demo-grid two'><article className='state-surface'><StateHeader icon='clock' title='Listă în încărcare' tag='Skeleton' /><div className='listing-skeleton-demo'>{[0,1,2].map((item) => <div key={item}><i /><span><b /><b /><b /></span></div>)}</div></article><article className='state-surface'><StateHeader icon='map' title='Hartă în încărcare' tag='Loading' /><div className='map-loading-demo'><div className='state-spinner' /><strong>Se pregătesc proprietățile…</strong><small>Harta rămâne în aceeași poziție</small></div></article></div></StateGroup>
      <StateGroup id='state-1' label='02 · Stări goale' title='Lipsa conținutului explică următorul pas'><div className='state-demo-grid three'>{[
        ['search','Niciun rezultat','Modifică prețul sau elimină un filtru pentru a vedea mai multe proprietăți.','Resetează filtrele'],
        ['filter','Filtrele nu se potrivesc','Încearcă fără „renovare recentă” sau mărește suprafața maximă.','Editează filtrele'],
        ['star','Încă nu există recenzii','Poți fi prima persoană care descrie o experiență verificabilă.','Scrie o recenzie'],
    ...
    {reportOpen && <div className='modal-backdrop' role='dialog' aria-modal='true' aria-labelledby='state-report-title'><form className='form-modal' onSubmit={(event) => { event.preventDefault(); setReportOpen(false); notify('Raportarea demonstrativă a fost trimisă') }}><button className='modal-close' type='button' aria-label='Închide' onClick={() => setReportOpen(false)}><Icon name='close' size={17} /></button><span className='section-kicker'>Siguranța comunității</span><h2 id='state-report-title'>Raportează o informație</h2><p>Alege motivul și descrie exact ce pare incorect.</p><label>Motiv<select><option>Preț incorect</option><option>Fotografii neactuale</option><option>Date personale</option><option>Conținut înșelător</option></select></label><label>Detalii<textarea placeholder='Descrie problema fără date personale inutile.' /></label><div className='modal-form-actions'><button type='button' onClick={() => setReportOpen(false)}>Anulează</button><button className='button button-primary' type='submit'>Trimite raportarea</button></div></form></div>}
  </div>
}

function StateHeader({ icon,title,tag }: { icon: IconName; title: string; tag: string }) { return <header className='state-card-header'><span><Icon name={icon} /></span><div><strong>{title}</strong><small>{tag}</small></div></header> }

function StateGroup({ id,label,title,children }: { id: string; label: string; title: string; children: React.ReactNode }) { return <section className='state-group' id={id}><header><span className='section-kicker'>{label}</span><h2>{title}</h2></header>{children}</section> }
