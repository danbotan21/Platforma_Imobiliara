...
export function GuidedFlowPage({ kind, navigate, notify }: GuidedFlowPageProps) {
  ...
  return (
    <div className="guided-flow-page">
      <header className="flow-header"><div className="container"><button type="button" onClick={() => navigate('/')}>← Renunță</button><div><span className="eyebrow">{info.eyebrow}</span><h1>{info.title}</h1><p>{info.description}</p></div><button type="button" onClick={() => notify('Progresul demonstrativ a fost salvat ca draft')}>Salvează ca draft</button></div></header>
      <div className="container flow-layout">
        ...
        <main className="flow-form-card">
          ...
          {kind === 'publish' && <PublishStep step={step} listingType={listingType} setListingType={setListingType} />}
          {kind === 'review' && <ReviewStep step={step} relation={relation} setRelation={setRelation} />}
          {kind === 'contribute' && <ContributionStep step={step} contribution={contribution} setContribution={setContribution} />}
      ...
      <FlowStateLibrary kind={kind} />
    </div>
  )
}

function FlowStateLibrary({ kind }: { kind: FlowKind }) {
...
function PublishStep({ step, listingType, setListingType }: PublishStepProps) {
  ...
  if (step === 5) return <div className="upload-section"><div className="upload-guidance"><strong>Fotografii ale proprietății</strong><p>Încarcă imagini luminoase și actuale. Nu adăuga documente personale.</p></div><div className="upload-grid"><button className="upload-success" type="button"><img src={galleryImages[0].src} alt="Imagine încărcată" /><span>✓ Imagine principală</span></button><button className="upload-success" type="button"><img src={galleryImages[1].src} alt="Imagine încărcată" /><span>✓ Încărcare reușită</span></button><button className="upload-empty" type="button"><span>＋</span><strong>Adaugă fotografii</strong><small>JPG sau PNG, maximum 15 MB</small></button></div><div className="before-after-upload"><strong>Fotografii înainte / după</strong><button type="button">＋ Adaugă set before/after</button></div></div>
  ...
  if (step === 8) return <div className="verification-panel"><span>▣</span><h3>Verificarea crește încrederea</h3><p>Documentele sunt folosite doar pentru confirmarea legăturii cu proprietatea și nu vor fi afișate public.</p><div className="upload-empty"><span>＋</span><strong>Încarcă documentul</strong><small>Contract, extras sau procură · datele pot fi mascate</small></div><label className="check-line"><input type="checkbox" /> Confirm că am dreptul să public acest anunț.</label></div>
  ...
  return <div className="final-check"><span>✓</span><h3>Anunțul este pregătit pentru trimitere</h3><p>Verifică încă o dată adresa, prețul și datele de contact. După trimitere, statusul va fi „În verificare”.</p><div><span>Conținut complet <b>100%</b></span><span>Fotografii <b>4</b></span><span>Document <b>încărcat</b></span></div></div>
}

interface ReviewStepProps { step: number; relation: string; setRelation: (value: string) => void }

function ReviewStep({ step, relation, setRelation }: ReviewStepProps) {
  if (step === 1) return <div className="form-grid"><label className="full-field">Caută proprietatea<input defaultValue="str. Lev Tolstoi 24/1" /></label><button className="selected-property" type="button"><img src={galleryImages[0].src} alt="Proprietate selectată" /><div><strong>str. Lev Tolstoi 24/1</strong><span>Centru · Toro Center</span></div><b>✓ Selectată</b></button></div>
  ...
  if (step === 4) return <div className="ratings-form">{['Starea apartamentului', 'Izolare fonică', 'Încălzire', 'Utilități', 'Proprietar', 'Vecini', 'Bloc', 'Zonă', 'Raport calitate/preț'].map((item) => <div key={item}><span>{item}</span><div><button type="button">★</button><button type="button">★</button><button type="button">★</button><button type="button">★</button><button className="muted-star" type="button">★</button></div><b>4/5</b></div>)}</div>
  ...
  if (step === 7) return <div className="upload-section"><div className="upload-grid"><button className="upload-empty" type="button"><span>＋</span><strong>Adaugă fotografii opționale</strong><small>Nu încărca documente în această zonă</small></button></div></div>
  if (step === 8) return <div className="verification-panel"><span>▣</span><h3>Verifică legătura cu proprietatea</h3><p>Poți încărca un extras anonimizat din contract sau solicita confirmarea proprietarului.</p><div className="choice-grid two"><button className="active" type="button"><strong>Document anonimizat</strong><p>Cea mai rapidă verificare.</p></button><button type="button"><strong>Confirmare de la proprietar</strong><p>Trimitem o solicitare separată.</p></button></div></div>
...
interface ContributionStepProps { step: number; contribution: string; setContribution: (value: string) => void }

function ContributionStep({ step, contribution, setContribution }: ContributionStepProps) {
